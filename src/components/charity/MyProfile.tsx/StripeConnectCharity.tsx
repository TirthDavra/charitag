...
import { IStripeDetails } from "@/components/merchant/MyAccount/types";
...
const StripeConnectCharity = ({
  ...
  const allStripeInfo = useAppSelectorCharity((state) => state.charityInfo);
  const stripeInfo = allStripeInfo.stripe.stripeDetails;

  useEffect(() => {
    if (!allStripeInfo.stripe.init) {
      dispatch(
        setCharityStripeInfo({ stripeDetails: initialState, init: true }),
      );
    }
  }, [dispatch]);
  ...
  return (
    <Formik
      ...
      onSubmit={async (values) => {
        ...
        response = await updateStripeDetailsCharity(values);
        ...
        if (response.error) {
          ...
          dispatch(updateCharityStripeInfo(values));
        }
      }}
    >
      {({ values }) => {
...
export default StripeConnectCharity;
