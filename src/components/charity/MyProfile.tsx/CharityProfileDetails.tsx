...
const CharityProfileDetails = ({ disabled }: { disabled: boolean }) => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    if (acceptedFiles.length > 0) {
      dispatch(
        updateCharityProfileImage(URL.createObjectURL(acceptedFiles[0])),
      );
    }
  }, []);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const imageUrl = response.data.user?.profile_image?.medium_path
          ...
          : null;
        dispatch(setCharityProfileImage({ profileImg: imageUrl, init: true }));
      } catch (error) {
