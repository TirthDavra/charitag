...
import { updateMerchantMemberStatus } from "@/api/merchant/merchantManageStaff";
import CustomTable from "@/components/merchant/Custom/Tables/CustomTable";
import {
  deleteCharityMember,
  updateCharityMemberStatus,
...
const CharityMembersTable = React.memo(
  ({ members, className, tableHeaders = true }: DataTableProps) => {
      ...
      openModal({
        content: (
          <ActionContent
            ...
            onCancel={closeModal}
            onOk={async () => {
              const response = await deleteCharityMember(id);
              if (!response.error) {
                setNewData(
                  produce((draft) => {
    ...
    const MembersColumnDef: ColumnDef<IMerchantMembers>[] = [
      {
        ...
        cell: ({ cell }) => {
          return (
            <div className="text-sm text-merchant_sidebar_text">
                  ...
                  openModal({
                    content: (
                      <ActionContent
                        ...
                        onCancel={closeModal}
                        onOk={async () => {
                          const response = await updateCharityMemberStatus({
    ...
    return (
      // <div>
      //   <CustomTable columns={MembersColumnDef} data={newData} />
      // </div>
      <div>
        <CustomRowRender
          columns={MembersColumnDef}
...
CharityMembersTable.displayName = "CharityMembersTable";
export default CharityMembersTable;
