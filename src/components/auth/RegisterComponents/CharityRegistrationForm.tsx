...
import { validationSchemaCharity } from "./ValiadtionSchemaForRegister";
import { SignUp, verifyCaptcha } from "@/api/auth/auth";
import { toast } from "react-toastify";
import { useRouter } from "next/navigation";
import ReCAPTCHA from "react-google-recaptcha";

interface ICharityRegistrationFormValues {
  first_name: string;
  last_name: string;
  charity_name: string;
  email: string;
  ...
  return (
    <div className="mt-4 flex flex-grow">
      <Formik
        ...
        validationSchema={validationSchemaCharity}
        onSubmit={async (
          values: ICharityRegistrationFormValues,
          { resetForm },
        ) => {
          const response = await SignUp({ ...values, type: 3 });
          if (!response.error) {
            ...
            <div>
              <div>
                <div className=" w-full rounded-xl  lg:mt-0 lg:max-w-[950px]">
                  <h2 className="mb-5 text-[24px] font-bold md:text-[24px]">
                    Create your charity login
                  </h2>
